...
public class AVLTree<E extends Comparable<E>> implements AVLTreeAPI<E> {
    ...
    public ArrayList<String> genPaths() {
         // calls the recursive function genPaths to generate paths of an array in ---> form.
    ...
    public boolean isFibonacci() {
       if(root == null){
         ...
         while(!node.isEmpty()){
            ...
            Node temp = node.poll();
            if(temp.bal== BalancedFactor.EH|| temp.bal== BalancedFactor.LH){
            return true;
            }
            if(temp.left != null){
            node.add(temp.left);
    ...
    public int fullCount() {
      if(root==null){
         return 0;
          ...
          Queue<Node> queueCount = new LinkedList<Node>();
         queueCount.add(root);
         int count = 0;
         while(!queueCount.isEmpty()){
          Node temp = queueCount.poll();
        if(temp.left!=null && temp.right!=null){
         count++;
         }
         if(temp.left!= null){
         queueCount.add(temp.left);
         }
         if(temp.right != null){
          queueCount.add(temp.right);
         }
         }
         return count;
    }
    ...
    private int height(Node node) {
       // returns the height iteratively
       int height = -1;
      if (root == null) {
         return -1;
     }
     ...
     while (!q.isEmpty()) {
         ...
         int size = q.size();
         while(size>0){
            Node nodess = q.remove();
            if (nodess.left != null){
                 q.add(nodess.left);
            }
            if (nodess.right != null){
               q.add(nodess.right);
          }
          size--;
